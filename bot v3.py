...
def bestColor(col): #col is tuple (b,g,r)
    minE = euc_col_dist_accurate(col,colors[0])
    bestCol=0
    for i in range(1,22):
        E=euc_col_dist_accurate(col,colors[i])
        if E<minE:
            minE=E
            bestCol=i
    return bestCol
def euc_col_dist_accurate(BGRcol,RGBcol):
    # https://en.wikipedia.org/wiki/Color_difference
    r=(BGRcol[2]+RGBcol[0])/2.0
    return (2+r/256.0)*(BGRcol[2]-RGBcol[0])*(BGRcol[2]-RGBcol[0])+4*(BGRcol[1]-RGBcol[1])*(BGRcol[1]-RGBcol[1])+(2+(255-r)/256.0)*(BGRcol[0]-RGBcol[2])*(BGRcol[0]-RGBcol[2])
...
for x in range(w):
        for y in range(h):
            c=int(drawMat[y,x])
            if c!=0 and isEdge(y,x,c):
                if col!=drawMat[y,x]:
                    mouse.position=(upleft[0]+colordx[c],upleft[1]+colordy[c])
                    col=c
                    mouse.click(Button.left,1)
                # mouse.position=(upleft[0]+psize*x,upleft[1]+psize*y)
                mouse.position=(upleft[0]+psize*(x+170),upleft[1]+psize*y)
                mouse.click(Button.left,1)
                fillMat[y,x]=-1
            if(x+2<w):
                drawMat[y,x+2]=bestColor(img[y,x])
